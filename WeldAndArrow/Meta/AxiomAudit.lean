...
def axiomAuditLedger : List AxiomAuditEntry := [
  ...
  { name := ``Grid.DirectedConvention.not_faithfulSpeechEnacted_of_no_faithful_utterance },
  ...
  { name := ``FaithNegative.effectiveTerminus_strictly_weaker_than_fullyEnlightened },
  { name := ``FaithNegative.fullyEnlightened_strictly_weaker_than_enacted },
  { name := ``FaithNegative.buddha_enacted_faces_absent_silent },
  { name := ``FaithNegative.EnactedWitness.fullyEnlightenedEnacted },
